...
- This command is only applicable to cluster NCPs.
...
.. - no command deletes the ncp entity from the database. For standalone cluster type, this configuration is invalid.

	- For **standalone cluster**, NCP 0 is configured by system. User cannot remove ncp-0 configuration.
...
.. **Help line:** configure a forwarding engine for DNOS cluster
