...
window.StackDialog = {
    showSystemStackDialog(editor, device, serial) {
        ...
        header.innerHTML = `
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <button id="stack-table-refresh" style="
                ...
                ">
                    <svg id="stack-refresh-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#d4a017" stroke-width="2.5">
        ...
        document.getElementById('stack-table-close').onclick = () => dialog.remove();
        ...
        const updateContent = (data) => {
            if (data.components && data.components.length > 0) {
                content.innerHTML = buildTableHtml(data.components);
                if (device) {
        ...
        refreshBtn.addEventListener('click', async () => {
            ...
            `;
            try {
                const data = await self._fetchStackLive(host, user, password, serial);
                updateContent(data);
                if (editor.showToast) editor.showToast('Stack data refreshed (live SSH)', 'success');
            } catch (err) {
                content.innerHTML = `
        ...
        self._fetchStack(host, user, password, serial, deviceLabel).then(data => {
            updateContent(data);
            const src = data.source || 'live';
            if (editor.showToast) editor.showToast(`Stack loaded (${src})`, 'info');
        }).catch(err => {
            content.innerHTML = `
                <div style="text-align:center; padding:40px; color:rgba(255,255,255,0.5);">
    ...
    async _fetchStack(host, user, password, serial, deviceLabel) {
        ...
        if (typeof ScalerAPI !== 'undefined' && ScalerAPI.getDeviceContext) {
            try {
                ...
                if (ctx?.stack && (Array.isArray(ctx.stack) ? ctx.stack : ctx.stack.components || []).length) {
                    ...
                    return { components, source: 'cached' };
                }
            } catch (_) {}
        }
        try {
            const data = await this._fetchStackLive(host, user, password, serial);
        ...
        if (typeof ScalerAPI !== 'undefined' && ScalerAPI.getDeviceContext) {
            try {
                ...
                if (ctx?.stack) {
                    const components = Array.isArray(ctx.stack) ? ctx.stack : (ctx.stack.components || []);
                    return { components, source: 'context' };
                }
            } catch (_) {}
        }
    ...
    async _fetchStackLive(host, user, password, serial) {
        const response = await fetch('/api/dnaas/device-stack-live', {
            method: 'POST',
