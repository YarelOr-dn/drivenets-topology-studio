...
**Description:** Enters bgp configuration under a non-default vrf network-service.

bgp as-number must be the same for all bgp instance vrf including default vrf

Possible BGP configuration are the same as "protocols bgp [as-number]" of default vrf, unless specified otherwise in the specific command
...
**BGP configuration under network-service vrf - configuration example:**
